<template>
    <v-dialog v-model="localIsModalVisible" max-width="600px" scrollable>
      <v-card class="card-custom elevation-12">
        <v-card-title class="headline">Foto de la INE</v-card-title>
        <v-card-text class="text-center">
          ...
          <v-img 
            ...
            contain
          ></v-img>
          <p class="text-body-1">Aquí está la imagen de tu INE.</p>
        </v-card-text>
  ...
  export default {
    ...
    watch: {
        isModalVisibleINE(newValue) {
        this.localIsModalVisible = newValue;
      },
      localIsModalVisible(newValue) {
        this.$emit('update:isModalVisibleINE', newValue);
      },
    },
    methods: {
      closeModal() {
